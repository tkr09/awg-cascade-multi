...
**v2.1.19** — исправлен сбор `ignoreip` в `awg-cascade-fail2ban.sh`. Первая
версия брала endpoint'ы со ВСЕХ интерфейсов, а на RU пиры `awg0` — это клиенты:
в список исключений попали шесть домашних IP пользователей. Это и бессмысленно
(оттуда никто не ломится в SSH), и растёт без границ, и записывает адреса людей
в конфиг. Теперь endpoint'ы собираются только с `awg-in*`, где пир заведомо
является RU, а внутренние адреса туннелей отбрасываются. Проверено: RU получает
свой адрес и адреса exit'ов, exit — свой и адреса обеих RU.
...
```bash
sudo awg-cascade-sync.sh --check v2.1.19   # показать дрейф
sudo awg-cascade-sync.sh v2.1.19           # привести ноду к тегу
```
