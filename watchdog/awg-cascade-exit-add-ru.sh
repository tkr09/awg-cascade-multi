...
# Бронь: если токен выдавался, он должен быть нашим. Но отсутствие СВОЕГО
# токена — сигнал более слабый, чем занятость индекса, и заваливать из-за него
# всю работу нельзя: к этому моменту exit уже полностью настроен, и повтор
# означает переделку с нуля. Так и случилось на заведении NL-1: бронь исчезла
# (её сняли снаружи), и провижининг, отработавший 15 минут, пропал впустую.
#
